...
export default function Projects() {
  ...
  return (
    <section id="projects" ref={sectionRef} className="py-20 px-4 sm:px-6 lg:px-8 bg-background relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div ref={cardsRef} className="grid md:grid-cols-2 gap-8" style={{ perspective: '1000px' }}>
          {projects.map((project, index) => (
            <Card key={index} className="project-card group h-full flex flex-col bg-card border-border hover:border-primary/20 transition-all duration-500 overflow-hidden relative" style={{ transformStyle: 'preserve-3d' }}>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <CardTitle className="text-2xl font-bold text-foreground group-hover:text-cyan-500 dark:group-hover:text-cyan-400 transition-colors">{project.title}</CardTitle>
                  <ArrowUpRight className="w-5 h-5 text-muted-foreground group-hover:text-cyan-500 dark:group-hover:text-cyan-400 group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" />
                </div>
              ...
              <CardContent className="flex-1 flex flex-col">
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.tags.map((tag) => (
                    <Badge key={tag} variant="outline" className="border-border text-muted-foreground hover:border-cyan-500/50 hover:text-cyan-500 dark:hover:text-cyan-400 transition-all">{tag}</Badge>
                  ))}
                </div>
